...
function startBroadcast() {
    ...
    // Ask the auctioneer for a custom code
    ...
    // Clean up the code to be safe for Firebase and easy to type
...
function placeBid(increment) {
    ...
    const team = getTeam(state.selectedTeamId);
      ...
      if (state.leadingTeamId === null) {
          ...
          nextBid = state.currentBid + 5;
      }
      if (nextBid > team.remainingBudget) { flash('Team over budget!'); return; }
      if (p.gender === 'Male' && team.maleSlots <= 0) { flash('No male slots left!'); return; }
      if (p.gender === 'Female' && team.femaleSlots <= 0) { flash('No female slots left!'); return; }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('bid1').onclick = () => placeBid(state.settings.step1);
    document.getElementById('bid10').onclick = () => placeBid(state.settings.step10);
    document.getElementById('bidCustom').onclick = () => {
      const val = Number(document.getElementById('customBid').value);
      if (val > 0) placeBid(val);
    };
    ...
    document.getElementById('saveSettings').onclick = ()=>{
      ...
      state.settings.step1 = Math.max(5, Number(document.getElementById('settingStep1').value)||5);
      state.settings.step10 = Math.max(10, Number(document.getElementById('settingStep10').value)||10);
      document.getElementById('bid1').textContent = `+ $${state.settings.step1}M`;
      document.getElementById('bid10').textContent = `+ $${state.settings.step10}M`;
    ...
    // Initialize button text from default settings
    document.getElementById('bid1').textContent = `+ $${state.settings.step1}M`;
    document.getElementById('bid10').textContent = `+ $${state.settings.step10}M`;
